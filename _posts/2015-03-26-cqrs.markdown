...
Before the event's are stored, The application publishes the event to an event bus so that all other systems/bounded contexts can subscribe to the event and interpret it accordingly. <br/>
